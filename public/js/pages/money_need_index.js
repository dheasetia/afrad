            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.status === 'success') {
                        ...
                        tr.append($('<td class="td_need" data-column="need">').text(data.need.description));
            ...
            success: function (data) {
                ...
                if (data.status === 'success') {
                    $.each(data.needs, function (index, value) {
                        ...
                        tr.append($('<td class="td_need" data-column="need">').text(value.description));
    ...
    var editItem = function () {
        ...
        function displayForm(cell) {
            var need_id = cell.closest('tr').data('id'),
                ...
                form =  '<form>' +
                    '<input type="text" name="description" class="form-control" value="' + prevContent + '"/>' +
        ...
        function changeField(cell, prevContent) {
            ...
            $.ajax({
                url: '/ajax/money-needs/edit',
                type: 'post',
                dataType: 'JSON',
                data: post_data,
                success: function (data) {
                    cell.html(data.need.description);
                    cell.off('click');
                    $('#panel_need').unblock();
                    $.toast({
                        heading: 'تعديل وظيفة',
