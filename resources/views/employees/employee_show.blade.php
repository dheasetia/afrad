        ...
        <div class="col-md-12">
            <div class="panel panel-info">
                <div class="panel-wrapper collapse in" aria-expanded="true">
                    <form action="{{url('employees', $employee->id)}}" method="post" accept-charset="utf-8"  id="form_create_employee">
                        {{csrf_field()}}
                        {{method_field('put')}}
                        ...
                        <div class="panel-body">
                                ...
                                <div class="row">
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            ...
                                            <select id="job_id" class="form-control" name="job_id">
                                                ...
                                                @if (count($jobs) > 0)
                                                    @foreach($jobs as $job)
                                                        <option {{old('job_id', $employee->job_id) == $job->id ? 'selected' : ''}} value="{{$job->id}}" >{{$job->job}}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                            ...
                                            <select id="department_id" class="form-control" name="department_id">
                                                ...
                                                @if (count($departments) > 0)
                                                    @foreach($departments as $department)
                                                        <option {{old('department_id', $employee->department_id) == $department->id ? 'selected' : ''}} value="{{$department->id}}" >{{$department->department}}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group {{$errors->has('mobile') ? ' has-error' : ''}}">
                                            ...
                                            <input type="text" id="mobile" name="mobile" value="{{old('mobile', $employee->mobile)}}" class="form-control">
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group {{$errors->has('email') ? ' has-error' : ''}}">
                                            ...
                                            <input type="text" id="email" name="email" value="{{old('email', $employee->email)}}" class="form-control">
                                            @if ($errors->has('email'))
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="form-group {{$errors->has('notes') ? ' has-error' : ''}}">
                                            ...
                                            <textarea class="form-control" id="notes" name="notes" rows="3">{{old('notes', $employee->notes)}}</textarea>
                                            @if ($errors->has('notes'))
                            ...
                            <div class="panel-footer">
                                <button type="submit" class="btn btn-success"> <i class="fa fa-save"></i>  حفظ التحديثات </button>
                            </div>
                        </div>
                    </form>
                </div>
